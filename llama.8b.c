/*
 * llama.c  —  Pure C inference engine for Llama 3.2-1B / Llama 3.1-8B
 *
 * Supports two weight file formats:
 *   1. .llmbin  — our simple flat binary (convert_weights.py → FAST, recommended)
 *   2. .safetensors — original HuggingFace format (slow JSON parse on startup)
 *
 * Models supported:
 *   - Llama 3.2-1B  (dim=2048,  layers=16, heads=32, kv_heads=8)
 *   - Llama 3.1-8B  (dim=4096,  layers=32, heads=32, kv_heads=8)
 *   Auto-detected from weight shapes.
 *
 * ─────────────────────────── Build ────────────────────────────────────
 *
 *  OpenBLAS (default):
 ...
 *    model.llmbin        (converted) OR model.safetensors
 *    tokenizer.json
 *
 * ─────────────────────── .llmbin format ────────────────────────────────
 *
 *  Offset 0   : magic    u32 = 0x4C4C4D42  ("LLMB")
 *  Offset 4   : version  u32 = 1
 *  Offset 8   : n_tensors u32
 *  Offset 12  : pad      u32 = 0
 *  Offset 16  : INDEX    n_tensors × 344 bytes each
 *    +  0  name     char[256]
 *    +256  dtype    u32  (0=F32, 1=BF16, 2=F16)
 *    +260  ndim     u32
 *    +264  shape    u64[8]
 *    +328  offset   u64  (from start of DATA section)
 *    +336  nbytes   u64
 *  After index (64-byte aligned): DATA
 *    Raw float32 bytes for each tensor (64-byte aligned each)
 */
...
static int json_read_string(const char *s, int i, char *out, int max) {
    if(s[i]!='"') return -1; i++;
    int j=0;
    while(s[i]&&s[i]!='"'){ if(s[i]=='\\') i++; if(j<max-1) out[j++]=s[i]; i++; }
    out[j]=0;
    return s[i]=='"' ? i+1 : -1;
}
static int json_read_uint64(const char *s, int i, uint64_t *v) {
    *v=0; if(s[i]<'0'||s[i]>'9') return -1;
    while(s[i]>='0'&&s[i]<='9'){ *v=*v*10+(s[i]-'0'); i++; } return i;
}

static int parse_tensor_entry(const char *hdr, int i, STTensor *t) {
    i=json_skip_ws(hdr,i); if(hdr[i]!='{') return -1; i++;
    memset(t,0,sizeof(*t)); t->dtype=ST_UNKNOWN;
    for(;;) {
        i=json_skip_ws(hdr,i);
        if(hdr[i]=='}'){i++;break;} if(hdr[i]==','){i++;continue;}
        char key[64]; i=json_read_string(hdr,i,key,sizeof(key)); if(i<0) return -1;
        i=json_skip_ws(hdr,i); if(hdr[i]!=':') return -1; i++;
        i=json_skip_ws(hdr,i);
        if(strcmp(key,"dtype")==0){
            char d[32]; i=json_read_string(hdr,i,d,sizeof(d)); if(i<0) return -1;
            if(!strcmp(d,"F32"))  t->dtype=ST_F32;
            else if(!strcmp(d,"BF16")) t->dtype=ST_BF16;
            else if(!strcmp(d,"F16"))  t->dtype=ST_F16;
            else if(!strcmp(d,"I32"))  t->dtype=ST_I32;
        } else if(strcmp(key,"shape")==0){
            if(hdr[i]!='[') return -1; i++;
            while(hdr[i]!=']'){
                i=json_skip_ws(hdr,i); if(hdr[i]==','){i++;continue;}
                uint64_t v; i=json_read_uint64(hdr,i,&v); if(i<0) return -1;
                if(t->ndim<8) t->shape[t->ndim++]=(int)v;
            } i++;
        } else if(strcmp(key,"data_offsets")==0){
            if(hdr[i]!='[') return -1; i++;
            i=json_skip_ws(hdr,i); i=json_read_uint64(hdr,i,&t->offset_start); if(i<0) return -1;
            i=json_skip_ws(hdr,i); if(hdr[i]==',') i++;
            i=json_skip_ws(hdr,i); i=json_read_uint64(hdr,i,&t->offset_end);   if(i<0) return -1;
            i=json_skip_ws(hdr,i); if(hdr[i]==']') i++;
        } else {
            if(hdr[i]=='"'){char tmp[256];i=json_read_string(hdr,i,tmp,sizeof(tmp));}
            else if(hdr[i]>='0'&&hdr[i]<='9'){uint64_t v;i=json_read_uint64(hdr,i,&v);}
            else while(hdr[i]&&hdr[i]!=','&&hdr[i]!='}') i++;
        }
    }
    return i;
}
...
static int st_open(SafeTensors *st, const char *path) {
    st->fd=open(path,O_RDONLY); if(st->fd<0){perror(path);return -1;}
    struct stat sb; fstat(st->fd,&sb); st->mmap_len=(size_t)sb.st_size;
    ...
    uint64_t hdr_len=0; for(int b=0;b<8;b++) hdr_len|=((uint64_t)raw[b])<<(8*b);
    char *hdr=( char*)raw+8; st->data=raw+8+hdr_len; st->data_len=st->mmap_len-8-hdr_len;
    int cap=512; st->tensors=(STTensor*)malloc(cap*sizeof(STTensor)); st->n=0;
    int i=0; i=json_skip_ws(hdr,i); if(hdr[i]!='{'){fprintf(stderr,"Bad header\n");return -1;} i++;
    for(;;){
        i=json_skip_ws(hdr,i); if(hdr[i]=='}') break; if(hdr[i]==','){i++;continue;}
        char tname[256]; int ni=json_read_string(hdr,i,tname,sizeof(tname)); if(ni<0) break; i=ni;
        i=json_skip_ws(hdr,i); if(hdr[i]!=':') break; i++;
        i=json_skip_ws(hdr,i);
        if(strcmp(tname,"__metadata__")==0){
            if(hdr[i]=='{'){int d=1;i++;while(hdr[i]&&d>0){if(hdr[i]=='{')d++;else if(hdr[i]=='}')d--;i++;}}
            continue;
        }
        if(st->n>=cap){cap*=2;st->tensors=(STTensor*)realloc(st->tensors,cap*sizeof(STTensor));}
        STTensor *t=&st->tensors[st->n];
        int ni2=parse_tensor_entry(hdr,i,t); if(ni2<0){fprintf(stderr,"Failed: %s\n",tname);break;}
        strncpy(t->name,tname,sizeof(t->name)-1); i=ni2; st->n++;
    }
    printf("[safetensors] Loaded %d tensors from %s\n",st->n,path);
    return 0;
}
static void st_close(SafeTensors *st){
    if(st->mmap_ptr&&st->mmap_ptr!=MAP_FAILED) munmap(st->mmap_ptr,st->mmap_len);
    if(st->fd>=0) close(st->fd); free(st->tensors);
}
static STTensor* st_find(SafeTensors *st, const char *name){
    for(int i=0;i<st->n;i++) if(!strcmp(st->tensors[i].name,name)) return &st->tensors[i];
    return NULL;
}
static float bf16_to_f32(uint16_t v){ uint32_t u=((uint32_t)v)<<16; float f; memcpy(&f,&u,4); return f; }
static float f16_to_f32_scalar(uint16_t v){
    uint32_t s=(v>>15)&1, e=(v>>10)&0x1f, m=v&0x3ff, u;
    if(e==0){ if(m==0){u=s<<31;}else{e=1;while(!(m&0x400)){m<<=1;e--;}m&=0x3ff;u=(s<<31)|((e+112)<<23)|(m<<13);} }
    else if(e==31){ u=(s<<31)|(0xff<<23)|(m<<13); }
...
static float* st_get_f32(SafeTensors *st, STTensor *t, float *scratch, int n){
    ...
    if(t->dtype==ST_F32) return (float*)ptr;
    if(t->dtype==ST_BF16){uint16_t *b=(uint16_t*)ptr;for(int i=0;i<n;i++)scratch[i]=bf16_to_f32(b[i]);return scratch;}
    if(t->dtype==ST_F16){uint16_t *b=(uint16_t*)ptr;for(int i=0;i<n;i++)scratch[i]=f16_to_f32_scalar(b[i]);return scratch;}
    fprintf(stderr,"Unsupported dtype for %s\n",t->name); return NULL;
}

static Config st_detect_config(SafeTensors *st) {
    STTensor *t = st_find(st, "model.embed_tokens.weight");
    if (t && t->ndim >= 2) {
        int dim = t->shape[1];
        if (dim == 4096) { printf("[st] Detected 8B config\n"); return llama31_8b_config(); }
    }
    printf("[st] Defaulting to 1B config\n");
    return llama32_1b_config();
}
...
/* ── safetensors path: malloc + copy ── */
static float* st_alloc_copy(SafeTensors *st, const char *name,
                              float **scratch, int *scratch_sz) {
    STTensor *t = st_find(st, name);
    if (!t) { fprintf(stderr,"Missing: %s\n",name); return NULL; }
    int n=1; for(int d=0;d<t->ndim;d++) n*=t->shape[d];
    float *buf=(float*)malloc((size_t)n*sizeof(float)); if(!buf) return NULL;
    if(t->dtype!=ST_F32 && n>*scratch_sz){
        float *ns=(float*)realloc(*scratch,(size_t)n*sizeof(float)); if(!ns){free(buf);return NULL;}
        *scratch=ns; *scratch_sz=n;
    }
    float *src=st_get_f32(st,t,*scratch,n); if(!src){free(buf);return NULL;}
    memcpy(buf,src,(size_t)n*sizeof(float)); return buf;
}

static int load_weights_st(Weights *w, Config *cfg, SafeTensors *st) {
    char name[256]; int scratch_sz=0; float *scratch=NULL;

#define COPY(dst,nm) do{ \
    (dst)=st_alloc_copy(st,(nm),&scratch,&scratch_sz); \
    ...
    for(int l=0;l<cfg->n_layers;l++){
#define LC(dst,fmt) snprintf(name,sizeof(name),fmt,l); COPY(dst[l],name)
        LC(w->attn_q,   "model.layers.%d.self_attn.q_proj.weight");
        LC(w->attn_k,   "model.layers.%d.self_attn.k_proj.weight");
        LC(w->attn_v,   "model.layers.%d.self_attn.v_proj.weight");
        LC(w->attn_o,   "model.layers.%d.self_attn.o_proj.weight");
        LC(w->ff_gate,  "model.layers.%d.mlp.gate_proj.weight");
        LC(w->ff_up,    "model.layers.%d.mlp.up_proj.weight");
        LC(w->ff_down,  "model.layers.%d.mlp.down_proj.weight");
        LC(w->attn_norm,"model.layers.%d.input_layernorm.weight");
        LC(w->ff_norm,  "model.layers.%d.post_attention_layernorm.weight");
#undef LC
    }
    COPY(w->norm,"model.norm.weight");

    STTensor *lm=st_find(st,"lm_head.weight");
    if(lm){ COPY(w->lm_head,"lm_head.weight"); printf("[weights] lm_head: separate\n"); }
    else {
        int n=cfg->vocab_size*cfg->dim;
        w->lm_head=(float*)malloc(n*sizeof(float));
        memcpy(w->lm_head,w->embed_tokens,n*sizeof(float));
        printf("[weights] lm_head: tied\n");
    }

    free(scratch);
    w->owns_memory=1;
    printf("[weights] All weights loaded.\n");
    return 0;
#undef COPY
}
