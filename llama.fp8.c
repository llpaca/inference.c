/*
 * llama.fp8.c - Pure C inference engine for Llama 3.2-1B
 * no difference form the other code it is just a fp8 version 
 * hoping to reduce the memory load
 */
...
/* this is will be the base for whole fp8*/
#include "QUANTIZATION/fp8.h"
...
static Config llama32_1b_config(void) {
    ...
    c.ff_dim      = 8192;
    c.vocab_size  = 128256;
    c.max_seq_len = 2048;   /* reduce for RAM; model supports 131072 */ ///@todo will update this based on user need 
...
static void fp8_rmsnorm(f8 *out, const f8 *x, const f8 *w, int dim){
    float ss = 0.0f;
    for(int i=0; i < dim; i++) ss += fp8_to_fp32(x[i]) * fp8_to_fp32(x[i]);
    ss = 1.0f / sqrtf(ss/(float)dim + 1e-5f);
    for(int i = 0; i < dim; i++) out[i] = fp8_mul(w[i], fp8_mul(fp32_to_fp8(ss),x[i]));
}
